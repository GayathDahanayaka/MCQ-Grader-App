                      ...
                      onTap: () {
                        if (!_serverOnline) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                "Server is offline! Check connection.",
                              ),
                            ),
                          );
                          return;
                        ...
                        _api.getAllResults().then((data) async {
                          if (data['success'] != false) {
                            ...
                            if (resultsRaw is List) {
                              ...
                              debugPrint(
                                "Warning: results is a Map, attempting to use values",
                              );
                              resultsList = resultsRaw.values
                                  .map((e) => Map<String, dynamic>.from(e))
                                  .toList();
                            }
                                    ...
                                    onExport: ({subject, gradeLevel, grade}) async {
                                      ...
                                      if (bytes != null && context.mounted) {
                                        try {
                                          ...
                                          final path =
                                              '${directory.path}/grading_results.xlsx';
                                          final file = File(path);
                                          await file.writeAsBytes(bytes);

                                          await Share.shareXFiles([
                                            XFile(path),
                                          ], text: 'Grading Results Export');
                                        } catch (e) {
                                          if (context.mounted) {
