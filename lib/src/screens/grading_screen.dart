  ...
  Future<void> _uploadMasterKey() async {
    ...
    if (_subjectController.text.isEmpty || _gradeLevelController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Please fill all master key details")),
      );
      return;
    }
    ...
    if (result['success'] != false) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Master Key Set Successfully!")),
      );
      _fetchMasterData();
      _clearImage(); // Reset for next action
      Navigator.pop(
        context,
      ); // Close dialog if open, though we might design this differently
    } else {
  ...
  Future<void> _gradeStudent() async {
    ...
    if (result['success'] != false) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => ResultsScreen(resultData: gradeData)),
      );
    } else {
      _showError(result['error']);
    }
                                ...
                                children: [
                                  if (!isMasterRecored)
                                    Container(
                                      ...
                                      child: const Text(
                                        "⚠️ No Master Key set! Please set one first.",
                                        style: TextStyle(
                                  ...
                                  ModernButton(
                                    ...
                                    onPressed: isMasterRecored
                                        ? _gradeStudent
                                        : () {}, // Disable if no master key
                              ...
                              child: Column(
                                children: [
                                  ...
                                  TextField(
                                    ...
                                    readOnly:
                                        true, // Make date picker clickable
                                    onTap: () async {
                                      DateTime? pickedDate =
                                      ...
                                      if (pickedDate != null) {
                                        _examDateController.text = pickedDate
                                            .toString()
                                            .split(' ')[0];
                                      }
                                    },
